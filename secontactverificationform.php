...
function handle_contact_post_verify() {
  ...
  if($nonce = $_POST["contact_form_verify_nonce"]){
    if(wp_verify_nonce($nonce, "cfn") ){
      ...
      if (get_option('isms_ip_address')=="1") {
      	$message .= "IP Address: " . getUserIP() . PHP_EOL;
      }
...
function register_mysettings_verify(){
  ...
  register_setting( 'email-sms-settings-group-verify', 'isms_ip_address' );
  ////
    ...
    <table>
      <tr>
        <td valign="top" width="50%">
            ...
            <div><label>IP Address</label><input name="isms_ip_address" type="checkbox" id="isms_ip_address" value="1" <?php echo get_option('isms_ip_address')=="1"?"checked":""; ?> /></div>
        </td>
        <td valign="top">
